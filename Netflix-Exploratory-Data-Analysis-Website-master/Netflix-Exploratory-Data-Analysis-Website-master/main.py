...
if b:
    if buttons == 'Netflix History':
        ...
        df11 = df10['release_year'].value_counts().reset_index().rename(
            columns={"index": "count", "release_year": "total"})
        fig1 = px.line(df11, x='total', y='count')
        st.plotly_chart(fig1)
        ...
        fig = px.treemap(df13, names=df13['Total_movies'], parents=["" for _ in df13['Total_movies']],
                         values=df13['count'],
